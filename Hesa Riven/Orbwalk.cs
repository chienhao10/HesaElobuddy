...
namespace Hesa_Riven
{
    ...
    public static class Orbwalking
    {
        ...
        {
            if (AfterAttack != null && target.IsValid)
            {
        ...
        {
            if (OnTargetChange != null && (!_lastTarget.IsValid || _lastTarget != newTarget))
            {
        ...
        public static bool InAutoAttackRange(AttackableUnit target)
        {
            ...
            //return Vector3.DistanceSquared((target is Obj_AI_Base) ? ((Obj_AI_Base)target).ServerPosition : target.Position, Player.ServerPosition) <= myRange * myRange;
        }
        ...
        {
            return Environment.TickCount >= LastAATick + Player.AttackDelay * 1000 && Attack;
        }
        ...
        public static bool CanMove(float extraWindup)
        {
            ...
            return (Environment.TickCount >= LastAATick + Player.AttackCastDelay * 1000 + extraWindup);
        }
            ...
            bool randomizeMinDistance = true)
        {
            ...
            {
                var movePath = Player.GetPath(point.To3D());
            ...
            if (Environment.TickCount - LastMoveCommandT < (70 + Math.Min(60, Game.Ping)) && !overrideTimer && angle < 60)
            {
                return;
            }

            if (angle >= 60 && Environment.TickCount - LastMoveCommandT < 60)
            {
                return;
            }

            Player.IssueOrder(GameObjectOrder.MoveTo, point);
            LastMoveCommandPosition = point.To3D();
        ...
        public static void Orbwalk(AttackableUnit target,
            ...
            bool randomizeMinDistance = true)
        {
            if (Environment.TickCount - LastAttackCommandT < (70 + Math.Min(60, Game.Ping)))
            {
                return;
            }
            ...
            try
            {
                if (target.IsValid && CanAttack())
                {
                    ...
                    if (!DisableNextAttack)
                    {
                        ...
                        {
                            LastAttackCommandT = Environment.TickCount;
                            _lastTarget = target;
                        }
                    }
                }
        ...
        {
            var spellbook = sender.Spellbook;
        ...
        public class Orbwalker
        {
            ...
            private bool ShouldWait()
            {
                return
                    ObjectManager.Get<Obj_AI_Minion>()
                        .Any(
                            minion =>
                                minion.IsValid && minion.Team != GameObjectTeam.Neutral &&
                                InAutoAttackRange(minion) && minion.IsMinion &&
                                HealthPrediction.LaneClearHealthPrediction(minion, (int)((Player.AttackDelay * 1000) * LaneClearWaitTimeMod), FarmDelay) <= Player.GetAutoAttackDamage(minion)
                        );
            }
            ...
            public virtual AttackableUnit GetTarget()
            {
                    ...
                    ActiveMode == OrbwalkingMode.LastHit)
                {
                    var MinionList =
                        ObjectManager.Get<Obj_AI_Minion>()
                            .Where(
                                minion =>
                                    minion.IsValid && InAutoAttackRange(minion))
                ...
                //Forced target
                if (_forcedTarget.IsValid && InAutoAttackRange(_forcedTarget))
                {
                    return _forcedTarget;
                }
                /* turrets / inhibitors / nexus */
                if (ActiveMode == OrbwalkingMode.LaneClear && (!_config.Item("FocusMinionsOverTurrets").GetValue<KeyBind>().Active || !EntityManager.MinionsAndMonsters.GetLaneMinions(EntityManager.UnitTeam.Enemy, ObjectManager.Player.Position, GetRealAutoAttackRange(ObjectManager.Player)).Any()))
                {
                    /* turrets */
                    foreach (var turret in
                        ObjectManager.Get<Obj_AI_Turret>().Where(t => t.IsValid && InAutoAttackRange(t)))
                    {
                        return turret;
                    }
                    ...
                    /* inhibitor */
                    foreach (var turret in
                        ObjectManager.Get<Obj_BarracksDampener>().Where(t => t.IsValid && InAutoAttackRange(t)))
                    {
                        return turret;
                    }
                    ...
                    foreach (var nexus in
                        ObjectManager.Get<Obj_HQ>().Where(t => t.IsValid && InAutoAttackRange(t)))
                    {
                        return nexus;
                    }
                }
                ...
                {
                    var target = TargetSelector.GetTarget(-1, DamageType.Physical);
                    if (target.IsValid && InAutoAttackRange(target))
                    {
                        return target;
                    }
                }
                ...
                if (ActiveMode == OrbwalkingMode.LaneClear || ActiveMode == OrbwalkingMode.Mixed)
                {
                    var jminions =
                        ObjectManager.Get<Obj_AI_Minion>()
                            .Where(
                                mob =>
                                mob.IsValid && mob.Team == GameObjectTeam.Neutral && this.InAutoAttackRange(mob)
                ...
                if (ActiveMode == OrbwalkingMode.LaneClear)
                {
                    if (!ShouldWait())
                    {
                        if (_prevMinion.IsValid && InAutoAttackRange(_prevMinion))
                        {
                        ...
                        result = (from minion in
                                      ObjectManager.Get<Obj_AI_Minion>()
                                          .Where(minion => minion.IsValid && InAutoAttackRange(minion) &&
                                          (_config.Item("AttackWards").GetValue<bool>() || !minion.IsWard()) &&
            ...
            private void DrawingOnOnDraw(EventArgs args)
            {
                ...
                {
                    foreach (var target in EntityManager.Enemies.FindAll(target => target.IsValidTarget(1175)))
                    {
                        Render.Circle.DrawCircle(
